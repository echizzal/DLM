...
void FeedDown_KB_KD(const int& SEED, const int& NumIter){
    DLM_DecayMatrix DECM;
    float Mass_K=493.677;
    float Mass_D=1869.65;
    float Mass_B=5279.34;
    float Mass_Dzero=1864.83;

    DECM.SetFileName(TString::Format("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/KB_KD_RS%i.root",SEED));
    DECM.SetHistoName("KB_KD");
    DECM.SetBins(1024,0,1024);
    DECM.SetNumDaughters1(1);
    DECM.SetMotherMass1(Mass_K);
    DECM.SetNumDaughters2(2);
    DECM.SetDaughterMass2(0,Mass_D);
    DECM.SetDaughterMass2(1,Mass_Dzero);
    DECM.SetMotherMass2(Mass_B);
    DECM.SetMeanMomentum(0);
    DECM.SetMomentumSpread(350);
    DECM.Run(SEED,NumIter);
}

void FeedDown_PiDstar_PiD(const int& SEED, const int& NumIter){
    DLM_DecayMatrix DECM;
    float Mass_Pi=139.57039;
    float Mass_D=1869.65;
    float Mass_pi0=134.9768;
    float Mass_Dstar=2010.26;

    DECM.SetFileName(TString::Format("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/Pidstar_PiD_RS%i.root",SEED));
    DECM.SetHistoName("Pidstar_PiD");
    DECM.SetBins(1024,0,1024);
    DECM.SetNumDaughters1(1);
    DECM.SetMotherMass1(Mass_Pi);
    DECM.SetNumDaughters2(2);
    DECM.SetDaughterMass2(0,Mass_D);
    DECM.SetDaughterMass2(1,Mass_pi0);
    DECM.SetMotherMass2(Mass_Dstar);
...
void FeedDown_PiB_PiD(const int& SEED, const int& NumIter){
    DLM_DecayMatrix DECM;
    float Mass_Pi=139.57039;
    float Mass_D=1869.65;
    float Mass_B=5279.34;
    ...
    DECM.SetFileName(TString::Format("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/PiB_PiD_RS%i.root",SEED));
    DECM.SetHistoName("PiB_PiD");
    DECM.SetBins(1024,0,1024);
    DECM.SetNumDaughters1(1);
    DECM.SetMotherMass1(Mass_Pi);
    DECM.SetNumDaughters2(2);
...
void FeedDown_CF_PiDstar_PiD_plus(const int& SEED){
    CATS Kitty;
    unsigned NumMomBins = 562;
    double kMin = 0;
    double kMax = 1024;

    double mass1 = 139.57039; //pion
    ...
    TH2F* histDecayKindematicsDstar=GetMatix(TString::Format("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/matrix/Pidstar_PiD_RS%i.root", SEED), "PiDstar_PiD");

    auto DLM_KDstarplus = new DLM_Ck(1, 0, Kitty);

    DLM_KDstarplus->Update();
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/Dstar_PiplusDplusOutput.root", "RECREATE");
    outputFile->cd();
...
void FeedDown_CF_PiDstar_PiD_minus(const int& SEED){
    CATS Kitty;
    ...
    CATSparameters source_func(CATSparameters::tSource, 1, true);
    Kitty.SetAnaSource(GaussSource, source_func);
    Kitty.SetAnaSource(0, 0.8132);//in fm the source size
    Kitty.SetQ1Q2(-1);//1 for same charge, -1 for opposite charged particle
    Kitty.SetQuantumStatistics(0);
    Kitty.SetRedMass(mass1 * mass2 /(mass1 + mass2));
    Kitty.SetNumChannels(1);
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/Dstar_PiplusDminusOutput.root", "RECREATE");
    outputFile->cd();
    grDstarminusSmeared->Write("smeared");
    grGenuine.Write();
    outputFile->Close();
...
void FeedDown_CF_KDstar_KD_minus(const int& SEED){
    CATS Kitty;
    unsigned NumMomBins = 128;
    double kMin = 0;
    double kMax = 1024;
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/Dstar_KplusDminusOutput.root", "RECREATE");
    outputFile->cd();
    grDstarminusSmeared->Write("smeared");
    grGenuine.Write();
    outputFile->Close();
...
void FeedDown_CF_PiB_PiD_plus(const int& SEED){
    CATS Kitty;
    unsigned NumMomBins = 128;
    double kMin = 0;
    double kMax = 1024;
    ...
    CATSparameters source_func(CATSparameters::tSource, 1, true);
    Kitty.SetAnaSource(GaussSource, source_func);
    Kitty.SetAnaSource(0, 0.8223);//in fm the source size
    Kitty.SetQ1Q2(1);//1 for same charge, -1 for opposite charged particle
    Kitty.SetQuantumStatistics(0);
    Kitty.SetRedMass(mass1 * mass2 /(mass1 + mass2));
    Kitty.SetNumChannels(1);
    ...
    TH2F* histDecayKindematicsDstar=GetMatix(TString::Format("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/matrix/PiB_PiD_RS%i.root", SEED), "PiB_PiD");
...
CkDec_KDstarplus_Smeared.Update();
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/B_PiplusDplusOutput.root", "RECREATE");
    outputFile->cd();
    grDstarminusSmeared->Write("smeared");
    grGenuine.Write();
    outputFile->Close();
...
void FeedDown_CF_PiB_PiD_minus(const int& SEED){
    CATS Kitty;
    unsigned NumMomBins = 128;
    double kMin = 0;
    double kMax = 1024;
    ...
    CATSparameters source_func(CATSparameters::tSource, 1, true);
    Kitty.SetAnaSource(GaussSource, source_func);
    Kitty.SetAnaSource(0, 0.8213);//in fm the source size
    Kitty.SetQ1Q2(-1);//1 for same charge, -1 for opposite charged particle
    Kitty.SetQuantumStatistics(0);
    Kitty.SetRedMass(mass1 * mass2 /(mass1 + mass2));
    Kitty.SetNumChannels(1);
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/B_PiplusDminusOutput.root", "RECREATE");
    outputFile->cd();
    grDstarminusSmeared->Write("smeared");
    grGenuine.Write();
    outputFile->Close();
...
void FeedDown_CF_KB_KD_plus(const int& SEED){
    CATS Kitty;
    unsigned NumMomBins = 128;
    double kMin = 0;
    double kMax = 1024;
    ...
    TH2F* histDecayKindematicsDstar=GetMatix(TString::Format("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/matrix/KB_KD_RS%i.root",SEED), "KB_KD");
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/B_KplusDplusOutput.root", "RECREATE");
    outputFile->cd();
    grDstarminusSmeared->Write("smeared");
    grGenuine.Write();
    outputFile->Close();
...
void FeedDown_CF_KB_KD_minus(const int& SEED){
    CATS Kitty;
    unsigned NumMomBins = 128;
    double kMin = 0;
    double kMax = 1024;
    ...
    auto outputFile = new TFile("/home/emma/CharmingAnalyses/DKDpi/oton_selections/Feeddown/B_KplusDminusOutput.root", "RECREATE");
    outputFile->cd();
    grDstarminusSmeared->Write("smeared");
    grGenuine.Write();
    outputFile->Close();
...
int maintwo(){
    ...
    // FeedDown_CF_KDstar_KD_minus(seed);

    // //  FeedDown_PiDstar_PiD(seed,100000000);
    //     std::cout<<"WOWOW"<<std::endl;

    // FeedDown_CF_PiDstar_PiD_plus(seed);
    // FeedDown_CF_PiDstar_PiD_minus(seed);

    // // FeedDown_KB_KD(seed,100000000);
    //     std::cout<<"WOWOW"<<std::endl;

    FeedDown_CF_KB_KD_plus(seed);
    FeedDown_CF_KB_KD_minus(seed);

    // FeedDown_PiB_PiD(seed,100000000);
        std::cout<<"WOWOW"<<std::endl;

    FeedDown_CF_PiB_PiD_plus(seed);
    FeedDown_CF_PiB_PiD_minus(seed);
